...
For an old Mac whose exact maintenance history is unknown, SSD Stage 6 first
...
| `GET /api/hub/stats[?hours=N]` | Generation analytics plus sanitized private fleet activity for Image generation, Voice generation, and Subtitle transcription: seven operational states, Hub/direct attribution, safe operation/job/model/progress/result evidence, bounded recent history, and honest partial/unknown mixed-version rows. Existing history defaults to 30 days and keeps like-for-like operation/model comparisons; prompts, transcripts, paths, media, handles, credentials, and full parameters are never returned. Select **View details** to make an authenticated, on-demand worker request for one exact job. |

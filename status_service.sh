#!/bin/bash
#
# Show whether the Studio Hub KH always-on service is actually running:
#   - is the server agent loaded + running (or crash-looping on a port clash)?
#   - is the watchdog scheduled?  (PERIODIC — runs ~1s every 60s.)
#   - is the Hub answering /api/health right now?
...
echo "═══════════ Studio Hub KH — service status ═══════════"
echo ""
...
if launchctl print "gui/$UID_NUM/$WD" >/dev/null 2>&1; then
  echo "   ✓ loaded · scheduled (runs ~1s each minute, so 'not running' between"
  echo "             checks is normal — it's working)"
else
  echo "   ✗ not loaded"
fi
echo ""
...
if curl -fsS --max-time 5 "http://127.0.0.1:$PORT/api/health" 2>/dev/null; then
  echo ""
  if [ "$CONFLICT" -eq 1 ]; then
    echo "   ⚠️  Port $PORT is answering — but it may be the OTHER instance"
    echo "      (Pinokio's Start), NOT the service. See the conflict fix below."
  else
    echo "   ✅ RUNNING — the Hub is up and responding on port $PORT."
  fi
else
  echo "   ⏳/❌ Not responding yet on port $PORT (still starting, stopped, or"
  echo "      the port is taken by Pinokio's Start)."
fi
echo ""
if [ "$CONFLICT" -eq 1 ]; then
  echo "──────────────────────────────────────────────────────────────"
  echo "⚠️  PORT CONFLICT — you're running BOTH the service AND Pinokio's Start."
  echo "   Pick ONE runner:"
  echo "   ▸ Let the SERVICE run it (recommended): stop Pinokio's Start —"
  echo "       pkill -f 'uvicorn backend.main:app.*$PORT'"
  echo "     …or Stop it in Pinokio. Within ~10s the service grabs the port."
  echo "   ▸ Go back to manual Start: click 'Uninstall Startup Service'."
  echo "──────────────────────────────────────────────────────────────"
  echo ""
fi

...
A separate coordinator identity-mutation fence covers Controller `site_id`,
`site_name`, and stable `controller_id` changes. Those change routes return busy
while any request is `redeemed`, `verifying`, or `confirmation_pending` without
authenticated `complete`, `never_applied`, or `needs_review` evidence. Explicit
cleanup may release the fence only after stopping/fencing the target Hub process
and verifying that it cannot still execute the accepted claim. The fence does
not block later repairs for different Macs. Fleet-token rotation does not take
this fence because repair neither returns nor writes the token.
...
Controller identity-mutation fence keeps the claim's site and Controller
snapshots unchanged while it is live. Later Macs may repair concurrently because
they have different target-local files and locks.
...
- Controller site/site-name/controller identity change endpoints remain busy for
  `redeemed`, `verifying`, or `confirmation_pending` until authenticated terminal
  evidence or explicit verified process-stop/fence cleanup. Restart never treats
  elapsed scheduling time as proof that a paused executor cannot write;
...
- Controller site/site-name/controller identity changes remain busy behind the
  live-claim fence through `confirmation_pending`; later different-Mac repair
  dispatches are not fenced;
...
A Controller site or stable-ID change rejected by the identity-mutation fence
says **Enrollment repair may still apply this location identity. Confirm the
repair result, or stop and verify the target Studio Hub before manual cleanup.**
It does not silently queue, guess completion, or cancel either operation.
...
changes nothing. A pre-replace hash mismatch does not call `os.replace`; it
...
| Controller site/Controller identity change after redemption | identity-mutation fence remains busy through verifying/confirmation-pending until terminal evidence or verified process-stop cleanup |
...
| suspended live executor | retains its one write and target-local managed-writer lock; Controller identity fence keeps claim values valid while later Macs continue |
...
| `controller_identity_change_busy` | site/site-name/controller change fenced pending terminal evidence or verified process stop | later repairs continue |
...
3. Release the Controller owner API, strict machine routes, Agent executor, and
   ...
   Controller site/site-name/controller changes through the coordinator
   identity-mutation fence. Do not change capability schema v3.
...
| Controller restart | queued order resumes; issued plaintext is not reconstructed; uncertain targets park as `confirmation_pending` after the scheduling interval; later different Macs continue; same target and Controller site/site-name/controller changes stay fenced until terminal evidence or verified process-stop cleanup; fleet-token rotation remains allowed |
...
| paused live executor | pause exactly before `os.replace`; Controller site change returns busy, next Mac dispatches and may repair, then resume performs the one valid write under unchanged site/controller snapshots |
| Controller mutation fence | site ID/name and stable Controller ID changes remain busy for redeemed/verifying/confirmation-pending work until authenticated `complete`/`never_applied`/`needs_review` or verified process-stop cleanup; fleet-token rotation proceeds and may force manual credential recovery for status |
...
| cleanup versus paused executor | cleanup/re-ticket and Controller site change fail while the executor may still be paused; after stop/fence plus verified process exit, the shared recovery procedure classifies state and safely releases the fence |
...
| races and overlap | concurrent target requests, token rotation, Controller identity change, registry edit/removal, expiry boundary, overlapping batches, paused executor, and restart during dispatch prove at most one fresh dispatch starts at a time; the unresolved same target/site mutation remains fenced while later Macs continue |
...
- Do Controller site/site-name/controller changes and same-target cleanup stay
  fenced until terminal evidence or verified process stop, while later Macs and
  fleet-token rotation remain unblocked?

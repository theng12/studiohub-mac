...
foreground outcome may classify its local file state or report terminal status
while a later Mac is dispatched. A `BEGIN IMMEDIATE` selection may move a queued
row to fresh-dispatch state only when no request is already in the foreground states
...
Controller operations that change the Controller's site identity, stable
Controller identity, or current fleet token share this coordinator foreground
lock. They return busy while any repair is `redeemed` or `verifying` inside its
15-second apply window. After that window the executor has no forward-write
authority, so those Controller changes may proceed without waiting for parked
status recovery.
...
the lock, so the executor's hash compare-and-swap detects them and never
overwrites a third state. Fleet-token writers do not take this local file lock
because repair never writes fleet-token files.
...
intent, or redirect/callback URL. `apply_window_seconds` is the fixed schema-1
design constant, not a renewable lease.
...
every field, including the exact fixed `apply_window_seconds = 15`. As one
durable transition it acquires the shared settings repair lock, re-reads
`controller_settings.json`, requires its hash to equal the journaled preimage,
captures the local monotonic start, fsyncs the accepted claim/state, and scrubs
the ticket. If the hash differs, it records `needs_review` without accepting the
claim or writing settings. The monotonic start is meaningful only to this live
process and is never reconstructed after restart.

The same foreground attempt checks the 15-second monotonic deadline before
staging. It constructs one full mode-0600 replacement that retains every
unlisted key and changes only:
...
the one full staged file, checks the monotonic deadline again immediately before the
write, re-reads `controller_settings.json`, and again requires the current hash
to equal the preimage. Expiry records `never_applied`; a third hash records
`needs_review`. Neither case calls `os.replace`. Only the exact preimage state
within the live 15-second window permits one `os.replace`, followed by directory
fsync and exact read-back.

No forward apply survives process restart, including a suspended process whose
monotonic clock advanced during `SIGSTOP` or sleep. Startup classifies the
single-file state without writing: current hash equal to staged plus exact
...
and begins the next eligible Mac. The parked Agent may independently classify
its single-file outcome or expose sanitized terminal status in the background.
The Controller may adopt that status later, but it cannot mint a new ticket for the same target
until it observes `complete` or `never_applied`, or the owner
...
operator to inspect the exact mode-0600 journal and current settings hash,
classify only staged/exact as complete or preimage/exact as never applied, scrub
...
- Controller site/controller identity and fleet-token change endpoints share
  the foreground coordinator lock and return busy for the live 15-second
  `redeemed`/`verifying` apply window. A Controller restart conservatively
  finishes that bounded window before allowing those changes; it never extends
  Agent authority;
...
- as part of durable claim acceptance the live process starts one 15-second
  monotonic deadline, takes the shared settings repair lock, and scrubs the
  ticket; sleep and `SIGSTOP` consume that window;
- it checks the deadline before staging and immediately before the one
  `os.replace`, with a preimage-hash compare-and-swap at both points;
...
- Controller site/controller or fleet-token rotation during the live 15-second
  apply window returns busy under the foreground coordinator lock;
...
- the bounded foreground interval prevents simultaneous fresh dispatch starts;
  ...
  different Macs continue even while that Agent classifies state or status is
  probed;
...
- every Studio Hub-managed writer of identity/parent settings shares the
  target-local repair file lock; an external writer is detected by the hash CAS
  and produces `needs_review`, never an overwrite.
...
| apply window expired | **The 15-second safety window expired. Nothing changed; retry.** |
...
A Controller site, stable-ID, or fleet-token change rejected by the foreground
lock says **Enrollment repair is applying identity. Try this change again in up
to 15 seconds.** It does not silently queue or cancel either operation.
...
changes nothing. An expired window or failed/ambiguous compare-and-swap does not
call `os.replace`; it records `never_applied` or `needs_review`. After a verified
single replacement, lack of a Controller response is confirmation uncertainty,
not a reason to write again; the status probe adopts it.
...
| Controller identity or token rotation during apply | change endpoint shares the foreground coordinator lock and returns busy for the live 15-second window |
| suspended or stale live executor | local monotonic time advances during sleep/`SIGSTOP`; checks before staging and `os.replace` prevent a post-window write |
...
| external settings writer | preimage hash compare-and-swap detects a third state and returns owner-visible `needs_review` without overwrite |
...
| `apply_window_expired` | never applied; no `os.replace` | continue; same target may retry after status adoption |
| `settings_writer_busy` | never applied/retryable; managed writer holds shared file lock | continue |
| `settings_preimage_changed` | needs review; external third state is not overwritten | continue |
| `controller_change_busy` | retry Controller site/controller/token change after 15-second foreground window | repair continues |
...
3. Release the Controller owner API, strict machine routes, Agent executor, and
   ...
   Controller site/controller/token changes through the coordinator foreground
   lock. Do not change capability schema v3.
...
| claim contents | response contains only bound identity fields and the exact constant `apply_window_seconds: 15`; it contains no fleet token, registration code, endpoint, redirect, owner credential, or workload/settings payload |
...
| Controller restart | queued order resumes; issued plaintext is not reconstructed; uncertain targets park as `confirmation_pending`; later different Macs continue after the bounded foreground interval; same target remains locked until authenticated `complete`/`never_applied` or explicit safe cleanup; site/controller/token changes remain conservatively busy through any surviving 15-second window |
...
| monotonic apply window | claim contains exactly `apply_window_seconds: 15`; sleep and `SIGSTOP` before staging or immediately before replacement advance monotonic time, expire to `never_applied`, and never call `os.replace`; restart never recreates the deadline or forward apply |
| Controller mutation fence | site, stable Controller identity, and fleet-token rotation/change endpoints return busy while a request is `redeemed`/`verifying` inside the 15-second window and succeed after it closes |
...
| external third state | manual edit between preimage capture and either CAS check yields `settings_preimage_changed`/`needs_review`, preserves the observed bytes, and performs no overwrite |
...
| batch | stable machine ordering and one-at-a-time fresh dispatch initiation; bounded uncertain Mac is parked, later Macs continue while target-local classification/status recovery may run, and the unresolved same target receives no new ticket until authenticated terminal evidence or explicit safe cleanup |
...
- Does the exact 15-second monotonic window fence sleep/`SIGSTOP`, with checks
  before staging and immediately before the one `os.replace`?
- Do all managed identity/parent writers share one local repair lock, does the
  hash CAS reject external third states, and do Controller site/controller/token
  changes return busy during the live apply window?

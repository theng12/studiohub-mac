...
class AutoUpdater:
    ...
    def _status_transaction(self):
        """Serialize status decisions and replacement across helper processes."""
        self.state_dir.mkdir(parents=True, exist_ok=True)
        with self._state_thread_lock:
            depth = getattr(self._state_depth, "value", 0)
            if depth:
                self._state_depth.value = depth + 1
                try:
                    yield
                finally:
                    self._state_depth.value -= 1
                return
            handle = open(self.state_lock_path, "a+", encoding="utf-8")
            try:
    ...
    def _write_status_unlocked(self, **changes: object) -> dict:
    ...
    def _write_status(self, **changes: object) -> dict:
        with self._status_transaction():
            return self._write_status_unlocked(**changes)
    ...
    def _active_managed_state(self, status: Optional[dict] = None) -> Optional[dict]:
        ...
        requested_commit = value.get("requested_commit", request["target_commit"])
        started_commit = value.get("started_commit")
        completed_commit = value.get("completed_commit")
        rollback_commit = value.get("rollback_commit")
        rollback_version = value.get("rollback_version")
        if requested_commit != request["target_commit"] or not COMMIT_RE.fullmatch(requested_commit):
            return None
        return {
            **request,
            ...
            **({"started_commit": started_commit} if isinstance(started_commit, str) and COMMIT_RE.fullmatch(started_commit) else {}),
            **({"completed_commit": completed_commit} if isinstance(completed_commit, str) and COMMIT_RE.fullmatch(completed_commit) else {}),
            **({"rollback_commit": rollback_commit} if isinstance(rollback_commit, str) and COMMIT_RE.fullmatch(rollback_commit) else {}),
    ...
    def _managed_history(self, status: Optional[dict] = None) -> list[dict]:
        ...
        for item in value:
            ...
            try:
                ...
                continue
            if request:
                state = self._active_managed_state({"active_managed_update": item})
                history.append({**request,
                                **({key: state[key] for key in ("requested_commit", "started_commit", "completed_commit", "rollback_commit")
                                    if state and key in state}),
                                "result": str(item.get("result") or "unknown")[:32]})
        return history
    ...
    def save_settings(self, payload: dict) -> dict:
        ...
        with self._status_transaction():
            ...
            if active:
                # The requested managed retry is authoritative over ordinary
                # scheduling settings, including an operator selecting Off.
                self._write_status_unlocked(pending_manual=True)
            else:
                next_check = None if mode == "off" else _iso(self._next_regular(saved))
                self._write_status_unlocked(next_check=next_check, defer_reason=None,
                                            pending_manual=False, state="idle")
        # apply_scheduler re-reads active managed state within its own
        # transaction. Do not carry this earlier snapshot across that boundary:
        # managed work can terminalize before the scheduler decision is made.
        ...
        if mode == "off" and applied["installed"] and not applied["forced_by_managed"]:
    ...
    def _write_wrapper(self) -> None:
        ...
        temporary = self.wrapper_path.with_name(
            f".{self.wrapper_path.name}.{os.getpid()}.tmp"
        )
    ...
    def apply_scheduler(self, *, force_pending: bool = False) -> dict:
        # A regular settings/scheduler caller may have observed no active
        # request before a managed admission. Re-read under the same lock used
        # by admission before any bootout/unlink decision.
        with self._status_transaction():
            forced_by_managed = bool(self._active_managed_request(self._read_status_unlocked()))
            force_pending = force_pending or forced_by_managed

            def scheduler_result() -> dict:
                # This decision is returned with the scheduler observation so
                # callers do not combine it with a later active-intent snapshot.
                return {**self.scheduler_status(), "forced_by_managed": forced_by_managed}
    ...
    def readiness_reasons(self, *, managed: bool = False) -> list[str]:
        ...
        try:
            ...
            if managed:
                if self._service_positively_stopped():
                    return []
                return ["the update safety check is unavailable and Studio Hub is not confirmed stopped"]
    ...
    def _admission_lock(self):
        """Compatibility name for the single re-entrant status transaction."""
        with self._status_transaction():
            yield
    ...
    def _port_accepting_connections(self) -> bool:
        try:
            with socket.create_connection(("127.0.0.1", int(self.spec["port"])), timeout=0.75):
                return True
        except OSError:
            return False
    ...
    def update(self, *, automatic: bool = False, target_commit: Optional[str] = None,
        ...
        managed = self._managed_request(target_commit=target_commit, target_version=target_version,
                                        operation_id=operation_id)
        with self._exclusive_lock():
            if not managed and self._active_managed_request():
                raise UpdateDeferred("A managed update operation is pending.")
            old_sha = ""
            new_sha = ""
            ...
            try:
                if managed:
                    # A trigger holds admission across the spawn/PID handoff, so
                    # re-read durable state only after that handoff completes.
                    with self._admission_lock():
                        ...
                        if active and self._active_managed_request() == managed:
                            ...
                            rollback = {
                                key: active[key] for key in ("started_commit", "completed_commit",
                                                              "rollback_commit", "rollback_version")
                                if key in active
                            }
                        else:
                ...
                if managed:
                    managed_state.setdefault("started_commit", old_sha)
                if managed and managed_state.get("rollback_commit"):
                    old_sha = managed_state["rollback_commit"]
                    old_version = managed_state.get("rollback_version", old_version)
                elif managed and preflight["available"]:
                    # Persist the only safe rollback point before stopping the
                    # run owner or changing the worktree.
                    managed_state.update(rollback_commit=old_sha, rollback_version=old_version)
                if managed:
                ...
                if not preflight["available"]:
                    if managed and managed_state["phase"] in {"stopping", "stopped", "merged"}:
                        ...
                        self._write_status(active_managed_update=managed_state)
                    if (managed and managed_state["phase"] in {"stopping", "stopped", "merged", "verified", "restarting"}
                            and mode != "stopped"):
                    ...
                    if managed:
                        managed_state["completed_commit"] = new_sha
                    self._write_status(state="succeeded", latest_version=preflight["latest"],
                                       ...
                                       **self._complete_managed_changes(managed_state or managed, "succeeded"))
                ...
                self._start_mode(mode)
                healthy = (self._verify_health(mode, preflight["latest"], new_sha)
                           if managed else self._verify_health(mode, preflight["latest"]))
                if not healthy:
                ...
                self._write_status(state="failed", last_update_result="Update failed",
                                   ...
                                   **self._complete_managed_changes(managed_state or managed, "failed"))
    ...
    def _start_managed_helper(self, managed: dict) -> dict:
        # This retry wins over a future ordinary schedule, including when normal
        # updater mode is Off, Notify, or Auto.
    ...
    def trigger_update(self, *, after_current: bool = False, target_commit: Optional[str] = None,
        ...
        with self._admission_lock():
            ...
            if managed:
                ...
                self._write_status(active_managed_update={**managed, "requested_commit": managed["target_commit"]},
                                   managed_helper_pid=None, pending_manual=True)
                if after_current:
                    reasons = self.readiness_reasons(managed=True)
                    ...
                    self.apply_scheduler(force_pending=True)
                    return self.public_status()
                # Persist and arm recovery before spawn: a killed helper or Hub
                # restart cannot lose a managed request while normal mode is Off.
                self.apply_scheduler(force_pending=True)
                return self._start_managed_helper(managed)
            if active:
    ...
    def scheduled(self) -> dict:
        with self._status_transaction():
            status = self._read_status_unlocked()
            cfg = self.settings()
            managed = self._active_managed_request(status)
            pending = bool(status.get("pending_manual")) or bool(managed)
        ...
        if pending:
            try:
                return self.update(automatic=False, **(managed or {}))
            except UpdateDeferred:
                return self.public_status()
        checked = self.check()
        with self._status_transaction():
            ...
            managed = self._active_managed_request(status)
            if not managed:
                self._write_status_unlocked(next_check=_iso(self._next_regular()), next_retry=None)
        if managed:
            try:
                return self.update(automatic=False, **managed)
            except UpdateDeferred:
                return self.public_status()

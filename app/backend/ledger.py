...
def stats(since_s: float | None = None, source: str = "all",
    ...
    with _conn() as conn:
        ...
        model_rows = conn.execute(
            ...
            f"GROUP BY model, operation ORDER BY model, operation", args).fetchall()
    ...
    model_stats = [
        {"model": r["model"], "count": r["c"], "avg_s": _round(r["avg_s"]),
         "modality": r["op"]}
        for r in model_rows if r["model"]
    ]
    # Keep the original map for older clients; model_rows is canonical because
    # one repository can legitimately serve more than one operation.
    by_model = {item["model"]: {key: value for key, value in item.items()
                                if key != "model"}
    ...
    return {"total": total, "by_machine": by_machine, "by_modality": by_modality,
            "by_model": by_model, "model_rows": model_stats,

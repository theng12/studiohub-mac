...
All remote work travels through :func:`peers.studio_request`, which routes a
registered remote Studio through its own authenticated Studio Hub.  The
controller's local Voice Studio is intentionally rejected as a target.
"""
...
async def _preflight(monitor, request: dict[str, Any], client: httpx.AsyncClient) -> Preflight:
    ...
    if machine == "local":
        raise QualificationError("LOCAL_TARGET_FORBIDDEN", "Qualification must run on a remote worker, never this controller machine.")
    if monitor.status.get(studio_id, {}).get("status") != "up":
        raise QualificationError("WORKER_NOT_IDLE", "The selected remote Voice Studio is not online and idle.")
    if studio_id in broker.busy_studios() or machine in broker.busy_machines():
        raise QualificationError("WORKER_NOT_IDLE", "The selected remote machine has active work.")
    if _active_attempt_on_machine(machine):
        raise QualificationError("WORKER_NOT_IDLE", "The selected remote machine already has a qualification attempt in progress.")
    if broker.machine_is_quarantined(machine):
        raise QualificationError("MACHINE_QUARANTINED", "The selected remote machine is temporarily quarantined.")
    ...
    snapshot = peers.cached(machine) or {}
    age = _peer_age(machine)
    ...
    if not isinstance(jobs, list):
        raise QualificationError("WORKER_IDLE_UNKNOWN", "The remote Voice Studio did not provide a safe idle-job snapshot.")
    if any(str(item.get("state") or "").lower() in {"queued", "running"}
           for item in jobs if isinstance(item, dict)):
        raise QualificationError("WORKER_NOT_IDLE", "The selected remote Voice Studio has active generation work.")

    return Preflight(studio=studio, model=entry, target={
        "site_id": control_plane.public_settings()["site_id"],
        "studio_id": studio_id,
        "machine_id": machine,
...
def _terminal_evidence(job: dict[str, Any]) -> dict[str, Any]:
    artifact = {key: job.get(key) for key in (
        "audio_duration_ms", "audio_duration_s", "audio_bytes", "audio_sha256",
        "media_type", "sample_rate_hz", "channels", "model_revision", "voice_revision",
...
async def submit(monitor, request: dict[str, Any], client: httpx.AsyncClient) -> dict[str, Any]:
    ...
    try:
        ...
        if reference_asset_id:
            ...
            }, files={"audio": (f"reference{reference['audio_extension']}", reference_path.read_bytes(), reference["media_type"])})
        else:
    ...
    except (httpx.HTTPError, OSError, execution_assets.ExecutionAssetError):
...
async def poll(monitor, attempt_id: str, client: httpx.AsyncClient) -> dict[str, Any]:
    ...
    if not studio or studio.get("machine", "local") == "local" or not attempt.get("worker_job_id"):

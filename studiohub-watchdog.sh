#!/bin/bash
#
# Studio Hub KH — health watchdog for the always-on server.
#
# launchd's KeepAlive restarts the Hub if the PROCESS dies, but it can't tell
# when the Hub is alive-but-hung. This script (run every 60s by the watchdog
...
FAILURE_FILE="${STUDIOHUB_WATCHDOG_STATE_FILE:-$ROOT/service/.watchdog-failures}"
FAILURES_REQUIRED="${STUDIOHUB_WATCHDOG_FAILURES_REQUIRED:-3}"
CURL_BIN="${STUDIOHUB_WATCHDOG_CURL_BIN:-curl}"
LAUNCHCTL_BIN="${STUDIOHUB_WATCHDOG_LAUNCHCTL_BIN:-launchctl}"
